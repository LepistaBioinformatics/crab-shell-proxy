...
// mediaFolderCaller decodes and authorizes, returning the workspace, the agent's
// harness and the body.
//
// The harness travels with the project because the two together name the
// directory: a project folder is a sibling of the main workspace under picoclaw
// and a child of it under the ganglion, so a folder created with the wrong one
// lands where the member's own agent cannot see it.
func (s *Server) mediaFolderCaller(w http.ResponseWriter, r *http.Request) (docker.WorkspaceKey, string, string, mediaFolderRequest, bool) {
	...
	return key, agent.Harness, req.Project, req, true
}
...
func (s *Server) handleMediaFolderCreate(w http.ResponseWriter, r *http.Request) {
	key, harness, project, req, ok := s.mediaFolderCaller(w, r)
	if !ok {
		return
	}
	if err := s.Mgr.CreateFolder(key, harness, project, mediaRelPath(req.Path)); err != nil {
...
func (s *Server) handleMediaMove(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.Mgr.MoveMedia(key, harness, project, mediaRelPath(req.Path), mediaRelPath(req.To)); err != nil {
		s.writeMediaFolderError(w, err)
		return
	}
...
func (s *Server) handleMediaFolderDelete(w http.ResponseWriter, r *http.Request) {
	...
	removed, err := s.Mgr.DeleteFolder(key, harness, project, mediaRelPath(req.Path))
	if err != nil {
		s.writeMediaFolderError(w, err)
		return
	}
