...
func (s *Server) handleAdminAgents(w http.ResponseWriter, r *http.Request) {
	...
	for _, key := range keys {
		...
		// applies to. The model inventory governs picoclaw, which is currently every
		// agent; the field is published so a client need not assume that stays true,
		// and so an older client's filter keeps working.
...
func (s *Server) handleAdminUserFilesList(w http.ResponseWriter, r *http.Request) {
	...
	files, err := s.Mgr.ListUserFiles(key, agent.Harness, "")
	if err != nil {
...
func (s *Server) handleAdminUserFilesDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.Mgr.DeleteUserFile(key, agent.Harness, "", name); err != nil {
		if errors.Is(err, docker.ErrMediaName) {
			writeJSON(w, http.StatusBadRequest, errBody(err.Error()))
			return
		}
